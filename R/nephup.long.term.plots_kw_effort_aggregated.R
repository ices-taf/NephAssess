...
#Landings trawl
  ...
  landings.st.nt <- rowSums(colSums(stock@landings[c("OTB_CRU", "OTT_CRU", "OTHER"),,,,,])) #all trawl landings
...
# Effort Scot NT in days & CPUE/LPUE SCOT NT kg/hour
  ...
  {
    tmp <- colSums(effort.data.days[c("OTB_CRU", "OTT_CRU", "OTB_DEF", "OTT_DEF"),,,,,])
  ...
  {
    tmp <- colSums(effort.data.kwdays[c("OTB_CRU", "OTT_CRU", "OTB_DEF", "OTT_DEF"),,,,,])
    effort.total <- rowSums(tmp)
    years.eff<- as.numeric(names(effort.total))
    years.match<- years.land[match(years.eff, years.land)]
    LPUE.tot <- landings.st.nt[names(landings.st.nt) %in% years.match]*1000/effort.total
  ...
  if(!is.null(effort.data.days) & !is.null(effort.data.kwdays))
  {
    ...
    effort.total<- lapply(tmp, function(x) {rowSums(colSums(x[c("OTB_CRU", "OTT_CRU", "OTB_DEF", "OTT_DEF"),,,,,]))})
...
########PLOTS##################
 ...
 #      out <- fname(i <- i + 1)
  ...
  if(international == T)
  {
	  ...
	  legend("topleft", legend=c("UK Scotland - All gears", "UK Scotland - trawlers", "Creel", "International"), pch=c(22,21,4,23), pt.bg = c("white", "white", "black", "black"), lty= 1, bty="n", cex=0.7)
  } else
  
  {
  legend("topleft", legend=c("UK Scotland - All gears", "UK Scotland - trawlers", "Creel"), pch=c(22,21,4), pt.bg = c("white", "white", "black"), lty= 1, bty="n", cex=0.6)
  }
  ...
  if(sum(c(is.null(effort.data.days), is.null(effort.data.kwdays)))==1)
  {
    ...
    title(main="Effort - Scottish trawlers", xlab="", ylab= ylabEff)

    # LPUE - Scottish trawlers
    ...
    title(main="LPUE - Scottish trawlers", xlab="", ylab= ylabLPUE)
  }
  ...
  if(!is.null(effort.data.days) & !is.null(effort.data.kwdays))
  {
    ...
    title(main="Effort - Scottish trawlers", xlab="")
