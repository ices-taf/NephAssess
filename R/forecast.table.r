...
forecast.table<- function(wk.dir, fu, hist.sum.table, land.wt.yrs, disc.wt.yrs, disc.rt.yrs, 
                          h.rates, d.surv =0.25)
{
...
#
  ...
  summary.output$total.catches.tonnes <-with(summary.output,landings.tonnes+dead.discards.tonnes+
                                                  surviving.discards.tonnes)
  
  
  out.sum <-vector("list")
  out.sum$table <-summary.output[,c(1,6,3,4,5,2)]
  out.sum$table[,2:5]<-lapply(out.sum$table[,2:5],round,0)
