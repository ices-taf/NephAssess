...
function(wk.dir, index = "index.txt", do.plots = TRUE, verbose = FALSE, na.rm = TRUE, old = TRUE)
{
 ...
 ## calculate info for count records
  ...
  if (old)
  {
	...
	average.density <- sapply(count.files, function(x) sum(c(x $ C1, x $ C2), na.rm = na.rm)) / viewed.area / 2
  ...
  } else
  {
    ...
    average.density <- ifelse( counter.1 + counter.2 == 0, 0, (counter.1 + counter.2) / viewed.area / 2)
    ...
    out <- 
      list(
        ...
        count.1 = counter.1.density,
        count.2 = counter.2.density
      )
  }
