create.international.landings<- function(wdir, fu, new.data.year, old.int.landings, new.scotland.landings, IC_file)
{
  ...
  official.Scotland<- scot[scot$Year==new.data.year,"Sub.Total"]
  ...
  land.by.country[land.by.country$Country %in% "UK(Scotland)","LANDINGS_OFFICIAL"]<- official.Scotland
  write.csv(land.by.country, file=paste0(wdir,"landings.by.country.csv"), row.names = FALSE)
  
  int[nrow(int)+1,] <- c(new.data.year,sum(land.by.country$LANDINGS_OFFICIAL))
